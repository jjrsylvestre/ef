...
\node at (0,1) (np) [vertexo,label=above:{$(0,1)$}] {};
\node at (0,0) (origin) [vertex,label=below:{$(0,0)$}] {};
\draw[->] (origin.west) -- (-1,0);
\draw[->] (origin.east) -- (1.75,0);
\node[vertex] at (0.5,0.5) (cpt) {};
\node at (1.25,1) (cptlabel) {$(x,y)$};
\draw[dashed] (cptlabel) -- (cpt);
\node[vertex] at (1,0) (gpt) {};
\node at (1,-0.5) (gptlabel) {$\bbrac{f(x,y),0}$};
